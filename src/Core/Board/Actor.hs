...
-}
data Actor = Player (Float, Float) Movement Movement | Computer (Float, Float) Movement [(Int, Int)]
...
position (Player   (a, b) _ _) = (fromIntegral (truncate a), fromIntegral (truncate b))
position (Computer (a, b) _ _) = (fromIntegral (truncate a), fromIntegral (truncate b))
